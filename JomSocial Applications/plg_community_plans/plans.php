...
if(!class_exists('plgCommunityPlans'))
{
	class plgCommunityPlans extends CApplications
	{
		var $_user		= null;
		var $_name		= "Plans";
		var $name		= "Plans"; //old versions
		var $_path		= '';
		var $db			= null;
		var $doc		= null;
		var $my			= null;
		...
		function plgCommunityPlans($subject, $config)
		{
		...
		function onProfileDisplay()
		{
			...
			//Load language file.
			JPlugin::loadLanguage( 'plg_plans', JPATH_ADMINISTRATOR );
			...
			if(!file_exists($this->_path . DIRECTORY_SEPARATOR . 'goals.php' ) ) {
				...
				$cache = JFactory::getCache('plgCommunityPlans');
				$cache->setCaching($caching);
				$callback = array('plgCommunityPlans', '_getGoalsHTML');
		...
		private function _getPlans()
		{
			...
			$query = GoalsHelper::getListQuery("plans",$this->db, $this->_user);
		...
		function _getGoalsHTML($items=null, $isOwner=false, $uid=0, $ownerMame='', $params=null)
		{
			...
			if ($isOwner) {
				...
				$html .= '<div class="gl_dashboard_buttons">
							<a class="gl_new_href" href="javascript:void(0);" onclick=\'javascript:goalsShowWindow("'.JRoute::_('index.php?option=com_goals&view=editplan&tmpl=component').'")\' >'.JHTML::tooltip(JText::_('COM_GOALS_NEW_PLAN_DESCR'), '', '', JText::_('COM_GOALS_NEW_PLAN')).'</a>
							<a class="gl_new_href" href="javascript:void(0);" onclick=\'javascript:goalsShowWindow("'.JRoute::_('index.php?option=com_goals&view=editstage&tmpl=component').'")\' >'.JHTML::tooltip(JText::_('COM_GOALS_NEW_STAGE_DESCR'), '', '', JText::_('COM_GOALS_NEW_STAGE')).'</a>
							<a class="gl_new_href" href="javascript:void(0);" onclick=\'javascript:goalsShowWindow("'.JRoute::_('index.php?option=com_goals&view=editplantask&tmpl=component').'")\' >'.JHTML::tooltip(JText::_('COM_GOALS_NEW_PLANTASK_DESCR'), '', '', JText::_('COM_GOALS_NEW_PLANTASK')).'</a>';
				$html .= '</div>';
				$html .= '<div class="gl_dashboard_buttons">';
				...
				$f_link = JRoute::_('index.php?option=com_goals&view=planuserfields&tmpl=component');
				$html .= '<a href="javascript:void(0);" onclick=\'javascript:goalsShowWindow("'.$f_link.'")\' class="gl_fields_href">'.JHTML::tooltip(JText::_('COM_GOALS_MY_FIELDS_PLANS'), '', '', JText::_('COM_GOALS_MY_FIELDS_PLANS')).'</a> ';
				$html .= '</div><br />';
			}
			...
			if(!empty($items)) {
				...
				foreach($items as $goal) {
					...
					$glink = JRoute::_('index.php?option=com_goals&view=plan&tmpl=component&id='.$goal->id.'&u='.$uid);
				...
				$html .= '<div class="gl_left_count">'.JText::_('PLG_GOALS_NO_PLANS').'</div>';
			}
