...
class GoalsViewEditRecord extends JViewLegacy
{
	protected $form;
	protected $item;
	protected $state;
	...
	public function display($tpl = null)
	{
		...
		$db = JFactory::getDbo();
        $getGoal = JRequest::getVar('gid');
        if ($getGoal) {
            $this->item->gid = $getGoal;
            
            $query = $db->getQuery(true);
            $query->select('metric, start, finish')->from('#__goals')->where('id='.$db->escape($getGoal));
            $response = $db->setQuery($query)->loadObject();
			
			$negative = false;
			//if($response->finish < $response->start) $negative = true;
			
			$this->item->metric = $response->metric;
			//$this->item->negative = $negative; 
        }

        $query = "SELECT id as value, title as text FROM `#__goals` WHERE `uid` = '".$user->id."'";
        $db->setQuery($query);
