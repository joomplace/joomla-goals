...
<script type="text/javascript">
	...
	{
		if (task == 'goal.cancel' || document.formvalidator.isValid(document.id('adminForm'))) {
...
<script type="text/javascript">
	...
	{
		var url="<?php echo JURI::root()?>/index.php?option=com_goals&task=goal.getuserfields&tmpl=component";
...
<div class="gl_dashboard form-horizontal">
	<h2 class="goals-form-name">
	 	<?php echo ($old)?JText::_('COM_GOALS_EDIT_FORM_TITLE_EDIT'):JText::_('COM_GOALS_EDIT_FORM_TITLE_NEW'); ?>
 	</h2>
    <div class="goals-form-set">
		...
		<?php if (GoalsHelperFE::getSettings()->allow_userfields) { ?>
    ...
    <div class="goals-form-actions control-group">
        <div class="controls">
        	<input type="button" class="btn" onclick="Joomla.submitbutton('goal.save')" value="<?php echo JText::_('JSAVE') ?>" />
        	<input type="button" class="btn" onclick="Joomla.submitbutton('goal.cancel')" value="<?php echo JText::_('JCANCEL') ?>" />
        </div>
    </div>
    <?php showJbField($this->form,'template'); ?>
    <?php showJbField($this->form,'featured'); ?>
	<input type="hidden" name="task" value="goal.edit" />
