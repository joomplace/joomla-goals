...
?>
<table class="table table-condenced" border="0" width="100%">
	<tr>
			 ...
			 <?php if (sizeof($items)) {
			  ...
			  <tr>
				  	...
				  	<td style="text-align:center;">
				  		<input type="button" class="btn btn-small" onclick="goalgoto('<?php echo JRoute::_('index.php?option=com_goals&view=editplanuserfield&id='.(int)$field->id.$tmpl);?>')" value="<?php echo JText::_('COM_GOAL_USERFIELD_EDITBUTTON'); ?>" />
						<input type="button" class="btn btn-small" onclick="if (confirm('<?php echo JText::_('COM_GOALS_DELETE_USERFIELD_MESS'); ?>'))goalgoto('<?php echo JRoute::_('index.php?option=com_goals&task=planuserfield.deletelog&id='.(int)$field->id.$tmpl);?>')" value="<?php echo JText::_('COM_GOALS_DELETE_RECORD'); ?>" />
				  	</td>
			  </tr>
			<?php
				}
				} else {?>
				<tr><td><div class="gl_msntf"><?php echo JText::_('COM_GOALS_FIELDS_NOT_FOUND');?></div></td></tr>
			<?php }?>
