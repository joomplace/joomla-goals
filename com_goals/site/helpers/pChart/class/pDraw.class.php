...
class pDraw
{
	...
	function drawRoundedRectangle($X1,$Y1,$X2,$Y2,$Radius,$Format="")
	{
		...
		if ( $Radius <= 0 ) {
			$this->drawRectangle($X1,$Y1,$X2,$Y2,$Color); return(0);
		}
	...
	function drawRoundedFilledRectangle($X1,$Y1,$X2,$Y2,$Radius,$Format="")
	{
		...
		if ( $Radius <= 0 ) {
			$this->drawFilledRectangle($X1,$Y1,$X2,$Y2,$Color); return(0);
		}
		...
		{
			$X1 = $Bounds["X1"]; $X1Dec = $this->getFirstDecimal($X1); if ( $X1Dec != 0 ) {
				$X1 = floor($X1)+1;
			}
			$X2 = $Bounds["X2"]; $X2Dec = $this->getFirstDecimal($X2); if ( $X2Dec != 0 ) {
				$X2 = floor($X2)-1;
			}
	...
	function drawFilledRectangle($X1,$Y1,$X2,$Y2,$Format="")
	{
		...
		{
			if ( $BorderR != -1 ) {
				$iX1=$X1+1; $iY1=$Y1+1; $iX2=$X2-1; $iY2=$Y2-1;
			} else { $iX1=$X1; $iY1=$Y1; $iX2=$X2; $iY2=$Y2;
			}
			...
			for($X=$iX1; $X<=$iX2+($iY2-$iY1); $X=$X+$DashStep)
			{
				$Y=$Y+$DashStep;
				if ( $X > $iX2 ) {
					$Xa = $X-($X-$iX2); $Ya = $iY1+($X-$iX2);
				} else { $Xa = $X; $Ya = $iY1;
				}
				if ( $Y > $iY2 ) {
					$Xb = $iX1+($Y-$iY2); $Yb = $Y-($Y-$iY2);
				} else { $Xb = $iX1; $Yb = $Y;
				}
	...
	function drawText($X,$Y,$Text,$Format="")
	{
		...
		if ( $BoxSurrounding != "" ) {
			$BoxBorderR = $BoxR - $BoxSurrounding; $BoxBorderG = $BoxG - $BoxSurrounding; $BoxBorderB = $BoxB - $BoxSurrounding; $BoxBorderAlpha = $BoxAlpha;
		}
		...
		if ( $DrawBox && ($Angle == 0 || $Angle == 90 || $Angle == 180 || $Angle == 270))
		{
			...
			if ( $Angle == 0 ) {
				$T[0]["X"]=-$TOffset;$T[0]["Y"]=$TOffset;$T[1]["X"]=$TOffset;$T[1]["Y"]=$TOffset;$T[2]["X"]=$TOffset;$T[2]["Y"]=-$TOffset;$T[3]["X"]=-$TOffset;$T[3]["Y"]=-$TOffset;
			}
	...
	function drawProgress($X,$Y,$Percent,$Format="")
	{
		...
		if ( $BoxSurrounding != NULL ) {
			$BoxBorderR = $BoxBackR + $Surrounding; $BoxBorderG = $BoxBackG + $Surrounding; $BoxBorderB = $BoxBackB + $Surrounding;
		}
	...
	function getLegendSize($Format="")
	{
		...
		$Boundaries = array(); $Boundaries["L"] = $X; $Boundaries["T"] = $Y; $Boundaries["R"] = 0; $Boundaries["B"] = 0;
		$vY = $Y; $vX = $X;
		foreach($Data["Series"] as $SerieName => $Serie)
		{
			if ( $Serie["isDrawable"] == TRUE && $SerieName != $Data["Abscissa"] )
		...
		foreach($Data["Axis"] as $AxisID => $Parameters)
		{
			...
			if ( $Parameters["Identity"] == AXIS_X )
			{
				if ( $Pos == SCALE_POS_LEFTRIGHT )
				{
					...
					{
						if ( $LabelRotation == 0 )					{
							$LabelAlign = TEXT_ALIGN_TOPMIDDLE; $YLabelOffset = 2;
						}
						if ( $LabelRotation > 0 && $LabelRotation < 190 )		{
							$LabelAlign = TEXT_ALIGN_MIDDLERIGHT; $YLabelOffset = 5;
						}
						if ( $LabelRotation == 180 )				{
							$LabelAlign = TEXT_ALIGN_BOTTOMMIDDLE; $YLabelOffset = 5;
						}
						if ( $LabelRotation > 180 && $LabelRotation < 360 )	{
							$LabelAlign = TEXT_ALIGN_MIDDLELEFT; $YLabelOffset = 2;
						}
					...
					{
						if ( $LabelRotation == 0 )					{
							$LabelAlign = TEXT_ALIGN_BOTTOMMIDDLE; $YLabelOffset = 2;
						}
						if ( $LabelRotation > 0 && $LabelRotation < 190 )		{
							$LabelAlign = TEXT_ALIGN_MIDDLELEFT; $YLabelOffset = 2;
						}
						if ( $LabelRotation == 180 )				{
							$LabelAlign = TEXT_ALIGN_TOPMIDDLE; $YLabelOffset = 5;
						}
						if ( $LabelRotation > 180 && $LabelRotation < 360 )	{
							$LabelAlign = TEXT_ALIGN_MIDDLERIGHT; $YLabelOffset = 5;
						}
				...
				elseif ( $Pos == SCALE_POS_TOPBOTTOM )
				{
					...
					{
						if ( $LabelRotation == 0 )					{
							$LabelAlign = TEXT_ALIGN_MIDDLERIGHT; $XLabelOffset = -2;
						}
						if ( $LabelRotation > 0 && $LabelRotation < 190 )		{
							$LabelAlign = TEXT_ALIGN_MIDDLERIGHT; $XLabelOffset = -6;
						}
						if ( $LabelRotation == 180 )				{
							$LabelAlign = TEXT_ALIGN_MIDDLELEFT; $XLabelOffset = -2;
						}
						if ( $LabelRotation > 180 && $LabelRotation < 360 )	{
							$LabelAlign = TEXT_ALIGN_MIDDLELEFT; $XLabelOffset = -5;
						}
					...
					elseif ( $Parameters["Position"] == AXIS_POSITION_RIGHT )
					{
						...
						if ( $LabelRotation > 0 && $LabelRotation < 190 )		{
							$LabelAlign = TEXT_ALIGN_MIDDLELEFT; $XLabelOffset = 6;
						}
						if ( $LabelRotation == 180 )				{
							$LabelAlign = TEXT_ALIGN_MIDDLERIGHT; $XLabelOffset = 5;
						}
						if ( $LabelRotation > 180 && $LabelRotation < 360 )	{
							$LabelAlign = TEXT_ALIGN_MIDDLERIGHT; $XLabelOffset = 7;
						}
	...
	function computeScale($XMin,$XMax,$MaxDivs,$Factors,$AxisID=0)
	{
		...
		foreach($GoodScaleFactors as $Key => $Factor)
		{
			if ( $Results[$Factor]["Rows"] > $MaxRows ) {
				$MaxRows = $Results[$Factor]["Rows"]; $BestFactor = $Factor;
			}
		}
	...
	function processScale($XMin,$XMax,$MaxDivs,$Factors,$AxisID)
	{
		...
		if ( $XMin != $XMax )
		{
				...
				foreach($Factors as $Key => $Factor)
				{
					if ( !$Found )
					{
						...
						if ( !$Found && floor($ScaleHeightRescaled/($Factor*$Scaled10Factor)) <= $MaxDivs ) {
							$Found = TRUE; $Rescaled = TRUE; $Result = $Factor * $Scaled10Factor;
						}
					}
				}
			...
			if ( $Rescaled ) {
				$XMin = $XMinRescaled; $XMax = $XMaxRescaled; $ScaleHeight = $ScaleHeightRescaled;
			}

			/* Compute rows size */
			$Rows      = floor($ScaleHeight / $Result); if ( $Rows == 0 ) {
				$Rows = 1;
			}
			...
			if ( $Mode == AXIS_FORMAT_METRIC && $Format == NULL )
			{
				...
				for($Decimals=0;$Decimals<=10;$Decimals++)
				{
					if ( !$Done )
					{
						...
						if ( $ScaleOK ) {
							$Done = TRUE; $GoodDecimals = $Decimals;
						}
					}
				}
		...
		foreach($Data["Series"] as $SerieName => $Serie)
		{
			if ( $Serie["isDrawable"] == TRUE && $SerieName != $Data["Abscissa"] && !isset($ExcludedSeries[$SerieName]))
			{
				...
				if ( $Data["Orientation"] == SCALE_POS_LEFTRIGHT )
				{
					...
					if ( $Type == BOUND_MAX || $Type == BOUND_BOTH )
					{
						if ( $MaxLabelPos == BOUND_LABEL_POS_TOP    || ( $MaxLabelPos ==  BOUND_LABEL_POS_AUTO && $MaxValue >= 0) ) {
							$YPos  = $PosArray[$MaxPos] - $DisplayOffset + 2; $Align = TEXT_ALIGN_BOTTOMMIDDLE;
						}
						if ( $MaxLabelPos == BOUND_LABEL_POS_BOTTOM || ( $MaxLabelPos ==  BOUND_LABEL_POS_AUTO && $MaxValue < 0) ) {
							$YPos  = $PosArray[$MaxPos] + $DisplayOffset + 2; $Align = TEXT_ALIGN_TOPMIDDLE;
						}
					...
					if ( $Type == BOUND_MIN || $Type == BOUND_BOTH )
					{
						if ( $MinLabelPos == BOUND_LABEL_POS_TOP    || ( $MinLabelPos ==  BOUND_LABEL_POS_AUTO && $MinValue >= 0) ) {
							$YPos  = $PosArray[$MinPos] - $DisplayOffset + 2; $Align = TEXT_ALIGN_BOTTOMMIDDLE;
						}
						if ( $MinLabelPos == BOUND_LABEL_POS_BOTTOM || ( $MinLabelPos ==  BOUND_LABEL_POS_AUTO && $MinValue < 0) ) {
							$YPos  = $PosArray[$MinPos] + $DisplayOffset + 2; $Align = TEXT_ALIGN_TOPMIDDLE;
						}
				...
				else
				{
					...
					{
						if ( $MaxLabelPos == BOUND_LABEL_POS_TOP    || ( $MaxLabelPos ==  BOUND_LABEL_POS_AUTO && $MaxValue >= 0) ) {
							$YPos  = $PosArray[$MaxPos] + $DisplayOffset + 2; $Align = TEXT_ALIGN_MIDDLELEFT;
						}
						if ( $MaxLabelPos == BOUND_LABEL_POS_BOTTOM || ( $MaxLabelPos ==  BOUND_LABEL_POS_AUTO && $MaxValue < 0) ) {
							$YPos  = $PosArray[$MaxPos] - $DisplayOffset + 2; $Align = TEXT_ALIGN_MIDDLERIGHT;
						}
					...
					{
						if ( $MinLabelPos == BOUND_LABEL_POS_TOP    || ( $MinLabelPos ==  BOUND_LABEL_POS_AUTO && $MinValue >= 0) ) {
							$YPos  = $PosArray[$MinPos] + $DisplayOffset + 2; $Align = TEXT_ALIGN_MIDDLELEFT;
						}
						if ( $MinLabelPos == BOUND_LABEL_POS_BOTTOM || ( $MinLabelPos ==  BOUND_LABEL_POS_AUTO && $MinValue < 0) ) {
							$YPos  = $PosArray[$MinPos] - $DisplayOffset + 2; $Align = TEXT_ALIGN_MIDDLERIGHT;
						}
		...
		foreach($Data["Series"] as $SerieName => $Serie)
		{
			if ( $Serie["isDrawable"] == TRUE && $SerieName != $Data["Abscissa"] )
			{
				...
				if ( $Data["Orientation"] == SCALE_POS_LEFTRIGHT )
				{
					...
					if ( $Picture != NULL ) {
						$PicOffset = $PicHeight / 2; $SerieWeight = 0;
					}
					$X = $this->GraphAreaX1 + $XMargin;

					if ( !is_array($PosArray) ) {
						$Value = $PosArray; $PosArray = array(); $PosArray[0] = $Value;
					}
				...
				else
				{
					...
					if ( $Picture != NULL ) {
						$PicOffset = $PicWidth / 2; $SerieWeight = 0;
					}
			...
			if ( $Serie["isDrawable"] == TRUE && $SerieName != $Data["Abscissa"] )
			{
				...
				else
				{
					...
					foreach($PosArray as $Key => $X)
					{
						...
						{
							if ( $X >= $LastX ) {
								$Align = TEXT_ALIGN_MIDDLELEFT; $Offset = $DisplayOffset;
							} else { $Align = TEXT_ALIGN_MIDDLERIGHT; $Offset = -$DisplayOffset;
							}
		...
		foreach($Data["Series"] as $SerieName => $Serie)
		{
			if ( $Serie["isDrawable"] == TRUE && $SerieName != $Data["Abscissa"] )
			{
				...
				if ( $InnerSurrounding != NULL ) {
					$InnerBorderR = $R+$InnerSurrounding; $InnerBorderG = $G+$InnerSurrounding; $InnerBorderB = $B+$InnerSurrounding;
				}
				if ( $InnerBorderR == -1 ) {
					$InnerColor = NULL;
					...
					foreach($PosArray as $Key => $Height)
					{
						if ( $Height != VOID && $Serie["Data"][$Key] != 0 )
						{
							...
							else
							{
								...
								if ( $InnerColor != NULL ) {
									$RestoreShadow = $this->Shadow; $this->Shadow = FALSE; $this->drawRectangle(min($X+$XOffset+1,$X+$XOffset+$XSize),min($Y1-$YSpaceUp+$YSpaceDown,$Y2)+1,max($X+$XOffset+1,$X+$XOffset+$XSize)-1,max($Y1-$YSpaceUp+$YSpaceDown,$Y2)-1,$InnerColor); $this->Shadow = $RestoreShadow;
								}
					...
					foreach($PosArray as $Key => $Width)
					{
						if ( $Width != VOID && $Serie["Data"][$Key] != 0 )
						{
							...
							else
							{
								...
								if ( $InnerColor != NULL ) {
									$RestoreShadow = $this->Shadow; $this->Shadow = FALSE; $this->drawRectangle(min($X1+$XSpaceLeft,$X2-$XSpaceRight)+1,min($Y+$YOffset,$Y+$YOffset+$YSize)+1,max($X1+$XSpaceLeft,$X2-$XSpaceRight)-1,max($Y+$YOffset,$Y+$YOffset+$YSize)-1,$InnerColor); $this->Shadow = $RestoreShadow;
								}
			...
			if ( $Serie["isDrawable"] == TRUE && $SerieName != $Data["Abscissa"] )
			{
				...
				if ( $Data["Orientation"] == SCALE_POS_LEFTRIGHT )
				{
					...
					foreach($PosArray as $Key => $Y)
					{
						...
						{
							$Slope = ($LastY - $Y); if ( $Slope > $MaxSlope ) {
								$MaxSlope = $Slope;
							} if ( $Slope < $MinSlope ) {
								$MinSlope = $Slope;
							}
				...
				else
				{
					...
					foreach($PosArray as $Key => $X)
					{
						if ( $X != VOID && $LastX != NULL )
						{
							$Slope = ($X - $LastX); if ( $Slope > $MaxSlope ) {
								$MaxSlope = $Slope;
							} if ( $Slope < $MinSlope ) {
								$MinSlope = $Slope;
							}
	...
	function drawBestFit($Format="")
	{
		...
		foreach($Data["Series"] as $SerieName => $Serie)
		{
			if ( $Serie["isDrawable"] == TRUE && $SerieName != $Data["Abscissa"] )
			{
				...
				} else { $R = $Serie["Color"]["R"]; $G = $Serie["Color"]["G"]; $B = $Serie["Color"]["B"];
				}
				...
				if ( $Data["Orientation"] == SCALE_POS_LEFTRIGHT )
				{
					...
					if ( $Y1 < $this->GraphAreaY1 ) {
						$X1 = $X1 + ($this->GraphAreaY1-$Y1); $Y1 = $this->GraphAreaY1;
					}
					if ( $Y1 > $this->GraphAreaY2 ) {
						$X1 = $X1 + ($Y1-$this->GraphAreaY2); $Y1 = $this->GraphAreaY2;
					}
					if ( $Y2 < $this->GraphAreaY1 ) {
						$X2 = $X2 - ($this->GraphAreaY1-$Y2); $Y2 = $this->GraphAreaY1;
					}
					if ( $Y2 > $this->GraphAreaY2 ) {
						$X2 = $X2 - ($Y2-$this->GraphAreaY2); $Y2 = $this->GraphAreaY2;
					}
	...
	function writeLabel($SeriesName,$Indexes,$Format="")
	{
		...
		if ( !is_array($Indexes) )    {
			$Index = $Indexes; $Indexes = array(); $Indexes[] = $Index;
		}
		if ( !is_array($SeriesName) ) {
			$SerieName = $SeriesName; $SeriesName = array(); $SeriesName[] = $SerieName;
		}
		if ( $ForceLabels != NULL && !is_array($ForceLabels) ) {
			$ForceLabel = $ForceLabels; $ForceLabels = array(); $ForceLabels[] = $ForceLabel;
		}
	...
	function drawLabelBox($X,$Y,$Title,$Captions,$Format="")
	{
		...
		if ( !$DrawSerieColor ) {
			$SerieBoxSize = 0; $SerieBoxSpacing = 0;
		}
	...
	function drawPolygonChart($Points,$Format="")
	{
		...
		foreach($Segments as $Key => $Pos)
		{
			...
			{
				if ( $inHorizon ) {
					$inHorizon = FALSE; $Result[] = array("X1"=>$LastX,"Y1"=>$Pos["Y1"],"X2"=>$Pos["X1"],"Y2"=>$Pos["Y1"]);
				}

				$Result[] = array("X1"=>$Pos["X1"],"Y1"=>$Pos["Y1"],"X2"=>$Pos["X2"],"Y2"=>$Pos["Y2"]);
			}
			else { if ( !$inHorizon ) {
				$inHorizon = TRUE; $LastX = $Pos["X1"];
			}
			}
		}
		$Segments = $Result;
		...
		if ( !$NoFill )
		{
			...
			for($Y=$MinY;$Y<=$MaxY;$Y=$Y+$YStep)
			{
				...
				if ( $RestoreLast != "-" ) {
					$Intersections[] = $RestoreLast; echo "@".$Y."\r\n";
				}
					...
					for($i=0;$i<=count($Intersections)-1;$i=$i+2)
					{
						if ( isset($Intersections[$i+1]) )
						{
							if ( $Intersections[$i] != $Intersections[$i+1] ) {
								$Result[] = $Intersections[$i]; $Result[] = $Intersections[$i+1];
							}
						}
					}
