...
class GoalsTableTask extends JTable
{
	function __construct(&$db)
	{
		parent::__construct('#__goals_tasks', 'id', $db);
	}
	...
	function store($updateNulls = false)
	{
		...
		if (parent::store())
		{
            ...
            GoalsHelper::updateGoal($gid);

            $query = $db->getQuery(true);
            $query->select('*');
            $query->from('#__goals');
            $query->where('`id`='.(int)$gid);
            $db->setQuery($query);
            $g = $db->loadObject();
		...
		$userfields = GoalsHelper::getCustomTaskUserFields($gid);
			...
			foreach ( $fileids as &$filid ) 
			{
                ...
                if($tmpname){
                    ...
                    if ($settings->file_exts)
                    {
                        ...
                        {$error=true; JError::raiseWarning(0,'Invalid ext. in '.$name);}
                    }
                    if ($settings->file_sizes)
                    {
                        if (($size/1024)>$settings->file_sizes)
                        {$error=true; JError::raiseWarning(0,'Invalid filesize: '.$name);}
                    }
                    }

                    if ($error==false)
                    {
                        $folder = JPATH_SITE.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'com_goals'.DIRECTORY_SEPARATOR.$user->id;
                        if (!JFolder::exists($folder))JFolder::create($folder);
                        $dest = $folder.DIRECTORY_SEPARATOR.$name;
                        $fvalues[$filid] = DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'com_goals'.DIRECTORY_SEPARATOR.$user->id.DIRECTORY_SEPARATOR.$name;
		...
		if (sizeof($ids))
		{
			foreach ( $ids as $fid ) 
			{
				...
				{
					$query = $db->getQuery(true);
					$query->update('#__goals_tasks_xref');
					$query->where('`fid`='.(int)$fid);
					$query->where('`tid`='.(int)$this->id);
				...
				{
					$query = $db->getQuery(true);
					$query->insert('#__goals_tasks_xref');
				...
				$db->setQuery($query);
				$db->query();
				echo 'SMT DEBUG: <pre>'; print_R(str_replace('#__', $db->getprefix(),$query->__toString())); echo '</pre>';
		...
		{
		$glink = 'index.php?option=com_goals&view=goal&id='.(int)$g->id;


		$templ_mes = $settings->jsoc_activity_mess_record;
		$a = $b = array();
		$a[] = '{record}'; 	$b[] = $this->title; 
		$a[] = '{value}';	$b[] = $this->value;
		$a[] = '{metric}';	$b[] = $g->metric;
		$a[] = '{goal}';	$b[] = '<a href="'.$glink.'">'.$g->title.'</a>';
    ...
    function delete($pk = null){
        parent::delete($pk);
        GoalsHelper::updateGoal($this->gid);
    }
}