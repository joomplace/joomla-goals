...
class GoalsTablePlanUserfield extends JTable
{
	function __construct(&$db)
	{
		parent::__construct('#__goals_plan_custom_fields', 'id', $db);
	}
	
	public function store($updateNulls = false)
	{
		if (!$this->user) $this->user=JFactory::getUser()->id;
		if (!$this->name) $this->name=str_replace('-','_',JFilterOutput::stringURLSafe($this->title));
				...
				if(sizeof($goals))
				{
					...
					{						
						$query = $db->getQuery(true);
							...
							$query->set('`fid`='.$db->quote($this->id).', `pid`='.$db->quote($gid));
							$db->setQuery($query);
						$db->query();
