...
class GoalsModelGoal extends JModelAdmin
{
	...
	{
		$form = $this->loadForm('com_goals.goal', 'goal', array('control' => 'jform', 'load_data' => false));
		if (empty($form)) {
			return false;
		}
        ...
        if (isset($post['template_name'])) {
            ...
            $item->template = $post['template_name'];
        } else {
            $item = $this->getItem();
        }
		$form->bind($item);
	...
	public function upload()
	{
		...
		?>
		<form method="post" action="<?php echo JRoute::_('index.php?option=com_goals&task=goal.upload&id='.(int) $id); ?>" enctype="multipart/form-data" name="filename">
