...
<form action="<?php echo JRoute::_('index.php?option=com_goals&view=plans'); ?>" method="post" name="adminForm" id="adminForm">
				...
				<table class="table table-striped">
						<thead>
							<tr>
                                ...
                                <th class="gl_left">
                                    <?php echo JHtml::_('grid.sort', 'COM_GOALS_CREATOR', 'creator', $listDirn, $listOrder); ?>
                                </th>
								<th class="gl_left">
						...
						<?php if (sizeof($this->goals)) {foreach($this->goals as $i => $item) {
							...
							<tr class="row<?php echo $i % 2; ?>">
								...
								<?php if ($this->canDo->get('core.edit') or ($this->canDo->get('core.edit.own') and $item->user_id == $this->user->get('id'))) { ?>
									<a href="<?php echo JRoute::_('index.php?option=com_goals&task=plan.edit&id='.$item->id);?>">
										<?php echo $this->escape($item->title); ?>
									</a>
								<?php } else { ?>
									<?php echo $this->escape($item->title); ?>
								<?php } ?>
								&nbsp;[<a href="<?php echo JRoute::_('index.php?option=com_goals&view=stages&filter_stages='.$item->id);?>"><?php echo JText::_('COM_GOALS_VIEW_STAGES');?></a>]
								</td>
                                <td align="center">
                                    <?php echo $item->featured?JText::_('COM_GOALS_YES'):JText::_('COM_GOALS_NO'); ?>
                                </td>
                                <td>
						...
						<?php }} else { ?>
							<tr>
								<td colspan="9" align="center" >
									<?php echo JText::sprintf('COM_GOALS_FIELD_NONE', 'plans'); ?>
									<a href="<?php echo JRoute::_('index.php?option=com_goals&task=plan.add'); ?>" >
