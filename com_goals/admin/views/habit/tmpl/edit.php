...
<script type="text/javascript">
    Joomla.submitbutton = function (task) {
        if (task == 'habit.cancel' || document.formvalidator.isValid(document.id('item-form'))) {
            Joomla.submitform(task, document.getElementById('item-form'));
        }
        else {
            if (!$('jform_title').get('value')) {
                alert('<?php echo JText::sprintf('COM_GOALS_ERROR_NOT_TITLE', 'habit'); ?>');
                $('jform_title').focus();
            }
        }
    }
...
<form action="<?php echo JRoute::_('index.php?option=com_goals&view=habit&layout=edit&id='.(int) $this->item->id); ?>" method="post" name="adminForm" id="item-form" class="form-validate form-horizontal" >
    <div class="row-fluid">
    <!-- Begin Content -->
